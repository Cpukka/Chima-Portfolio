   ...
   spaceBetween={10}
    ...
    {workSlider.slides.map((slide, index) => {
        return (
          ...
          <div className='grid grid-cols-2 grid-rows-2 gap-4 cursor-pointer pr-5'>
            {slide.images.map((image, index)=> {
              return (
                <div className=' relative rounded-lg overflow-hidden flex
                item-center justify-center group' key={index}>
                  <div className=' flex items-center justify-center relative overflow-hidden group'>
                    {/** image*/}
                    <Image src={image.path} width={500} height={300} alt='' />
                    {/**overlay gradient */}
                    <div className='absolute inset-0 bg-gradient-to-l from-transparent via-pink-400 to-fuchsia-700
